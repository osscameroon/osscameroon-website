...
_export(){
    for i in {1..1500}; do
        save_on="./data/users_page_$i.json"
        echo "Exporting users, page $i"

        curl -Ls 'https://api.osscameroon.com/github/users/search' -X POST \
            ...
            --data-raw '{"count":21,"page": '$i'}' | jq > $save_on;
